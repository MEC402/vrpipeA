...
def printPano(panofile,aname,anavlist,aicon):
  ...
  print ("{\"id\": \""+idname+"\",")
  print (" \"name\": \""+aname+"\",")
  print (" \"left\": \""+rpanofile+"/left/%d/%d/%d/%d.jpg\",")
  print (" \"right\": \""+rpanofile+"/right/%d/%d/%d/%d.jpg\",")
  print (" \"thumb\": \""+rpanofile+"/left/1/f/0/0.jpg\",")
  print (" \"icon\": \""+aicon+"\",")
  if (len(anavlist) > 1):
     yvnl = anavlist[2];
  ...
  for anl in anavlist[3:]:
      ayaw = gyaw;
      apitch = 0.0;
      acon = "icon-casa-serena-dining-room-128x128.png";
      if "yaw" in anl:
          ayaw = anl["yaw"];
          apitch = anl["pitch"];
      if "icon" in anl:
